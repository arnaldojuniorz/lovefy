...
const BASE_URL    = (process.env.NEXT_PUBLIC_APP_URL ?? 'https://www.lovefy.app.br').replace(/\/+$/, '')
...
// Navigator com tipagem correta para Web Share API
...
function normalizeFotos(value: unknown): FotoComUrl[] {
  ...
  for (const item of value) {
    ...
    out.push({ ...foto, _url: url, _ordem: Number.isFinite(ordemNum) ? ordemNum : 9999 })
  }
...
const STYLES = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .grad-pill { background: linear-gradient(135deg,#FF2D7A,#7928FF); background-size:200% 200%; animation: gradAnim 4s ease infinite; }
  .section-card { border-radius: 20px; overflow: hidden; margin-bottom: 12px; }
...
function FloatingPlayer({ carta, spotifyId }: { carta: CartaViewerData; spotifyId: string | null }) {
  ...
  if (!spotifyId) {
    return (
      <div className="slide-up" style={{ position: 'fixed', bottom: 20, right: 16, zIndex: 200, background: 'linear-gradient(135deg,#1a4a6e,#0d2d45)', borderRadius: 40, boxShadow: '0 8px 32px rgba(0,0,0,0.6)', border: '1px solid rgba(255,255,255,0.1)' }}>
        <button onClick={() => window.open(musicaLink, '_blank')} aria-label="Abrir música no Spotify"
  ...
  return (
    <div className="slide-up" style={{ position: 'fixed', bottom: 20, right: 16, zIndex: 200, borderRadius: minimizado ? 40 : 16, boxShadow: '0 8px 32px rgba(0,0,0,0.7)', border: '1px solid rgba(255,255,255,0.15)', overflow: 'hidden', transition: 'all 0.3s cubic-bezier(.34,1.56,.64,1)', width: minimizado ? 56 : 'calc(100vw - 32px)', maxWidth: minimizado ? 56 : 340, background: '#121212' }}>
      {minimizado ? (
        <button onClick={() => setMinimizado(false)} aria-label="Expandir player"
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px 0' }}>
            ...
            <button onClick={() => setMinimizado(true)} aria-label="Minimizar player"
          ...
          <iframe
            ...
            loading="eager" title="Player de música"
...
function HeroSection({ carta, fotoUrl }: { carta: CartaViewerData; fotoUrl: string | null }) {
  return (
    <div style={{ position: 'relative', height: '100vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      {fotoUrl ? (
        <img src={fotoUrl} alt="" aria-hidden="true" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', filter: 'brightness(0.3)' }} />
      ) : (
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px 0' }}>
        ...
        <div className="grad-pill" style={{ color: '#fff', fontWeight: 800, fontSize: 12, padding: '5px 16px', borderRadius: 100, fontFamily: 'Poppins, sans-serif', letterSpacing: 1 }}>Wrapped</div>
        <div style={{ width: 36 }} />
      </div>
...
function MapaSection({ carta }: { carta: CartaViewerData }) {
  ...
  useEffect(() => {
    if (mapaInicial || !dataImportante || !cartaId) { setLoading(false); return }
    fetch('/api/mapa-estrelas', {
      ...
      .then((r) => r.json())
      .then((d) => { if (d?.imageUrl) setMapaUrl(String(d.imageUrl)) })
      .catch(() => {})
  ...
  return (
    <div className="section-card" style={{ background: 'linear-gradient(180deg, #05051A 0%, #0D0D2F 100%)', padding: '32px 20px 36px', textAlign: 'center' }}>
      ...
      <p style={{ color: '#fff', fontSize: 20, fontWeight: 700, fontFamily: 'Poppins, sans-serif', marginBottom: 28, lineHeight: 1.3 }}>O céu na noite<br />de vocês</p>
...
function GaleriaSection({ carta, fotos }: { carta: CartaViewerData; fotos: FotoComUrl[] }) {
  ...
  return (
    <div className="section-card" style={{ background: '#1A1A1A', paddingTop: 20, paddingBottom: 20 }}>
      ...
      {lightbox && (
        <div onClick={() => setLightbox(null)} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.96)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <img src={lightbox} alt="Foto em tamanho completo" style={{ maxWidth: '100%', maxHeight: '90vh', borderRadius: 16, objectFit: 'contain', animation: 'lightboxIn 0.3s ease' }} />
      ...
      <div style={scrollStyle}>
        {tresfotos.map((foto, idx) => (
          <div key={String(foto.id ?? idx)} onClick={() => { setAtiva(idx); setLightbox(foto._url) }}
            ...
            <img src={foto._url} alt={`Foto ${idx + 1}`}
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: 6, marginTop: 14 }} role="tablist" aria-label="Navegação de fotos">
        {tresfotos.map((_, idx) => (
          <div key={idx} onClick={() => setAtiva(idx)} role="tab" aria-selected={idx === ativa}
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
            {palavras.map((p) => {
              ...
              return (
                <div key={p} style={{ background: '#262626', borderRadius: 12, padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: `1px solid ${jaAcertou ? 'rgba(255,45,122,0.4)' : 'rgba(255,255,255,0.06)'}` }}>
                  <div>
                    <p style={{ color: jaAcertou ? '#FF2D7A' : 'rgba(255,255,255,0.2)', fontSize: 16, fontWeight: 700, letterSpacing: jaAcertou ? 0 : 5, fontFamily: 'Poppins, sans-serif' }}>
                      {jaAcertou ? p : '•'.repeat(p.length)}
                    </p>
                    {!jaAcertou && <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: 11, marginTop: 2 }}>Dica: {p.length} letras</p>}
                  </div>
...
function WrappedCard({ carta, fotoUrl }: { carta: CartaViewerData; fotoUrl: string | null }) {
  ...
  const cartaUrl         = `${BASE_URL}/c/${slug}`
  ...
  async function salvarImagem() {
    ...
    try {
      ...
      ctx.fillText(`${BASE_URL.replace('https://', '')}/c/${slug}`, 540, 1750)
      ...
      const payload: ShareData = { files: [file], title: 'Wrapped Lovefy', text: `Ver aqui: ${cartaUrl}` }
      ...
      console.error('[WrappedCard] erro ao salvar imagem:', err)
    } finally {
      setSalvando(false)
    }
  }
  ...
  return (
    <div className="section-card" style={{ marginBottom: 12, overflow: 'hidden' }}>
      <div style={{ position: 'relative', background: 'linear-gradient(135deg, #FF2D7A 0%, #7928FF 60%, #0D0D0D 100%)', padding: '40px 28px 32px', textAlign: 'center', overflow: 'hidden' }}>
        {fotoUrl && (
          <img src={fotoUrl} alt="" aria-hidden="true"
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 12, padding: '10px 20px', display: 'inline-block' }}>
            ...
            <p style={{ color: '#fff', fontSize: 13, fontWeight: 600 }}>{BASE_URL.replace('https://', '')}/c/{slug}</p>
          </div>
        </div>
      </div>
...
function CTASection({ carta }: { carta: CartaViewerData }) {
  return (
    <div className="section-card" style={{ background: '#1A1A1A', padding: '28px 20px', textAlign: 'center' }}>
      ...
      <p style={{ color: 'rgba(255,255,255,0.15)', fontSize: 11, marginTop: 16 }}>{BASE_URL.replace('https://', '')}</p>
    </div>
  )
}