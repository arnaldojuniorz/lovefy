...
export default function CartaViewer({ carta }: { carta: Carta }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0D0D0D', paddingBottom: 48 }}>
      ...
      {/* ✅ Player flutuante — renderiza antes de tudo, toca automaticamente */}
      ...
      <div style={{ padding: '0 12px' }}>
        {/* ✅ Foto de destaque simples — sem layout de player */}
...
function FloatingPlayer({ carta, fotoUrl, spotifyId }: { carta: Carta; fotoUrl: string | null; spotifyId: string | null }) {
  const [tocando, setTocando] = useState(true) // ✅ autoplay
  ...
  return (
    <>
      {/* ✅ Player flutuante canto inferior direito */}
      <div className="slide-up" style={{
        ...
        borderRadius: minimizado ? 40 : 20,
        boxShadow: '0 8px 32px rgba(0,0,0,0.6)',
        border: '1px solid rgba(255,255,255,0.1)',
        transition: 'all 0.3s cubic-bezier(.34,1.56,.64,1)',
        overflow: 'hidden',
        width: minimizado ? 56 : 220,
      }}>
        {minimizado ? (
          ...
          <div style={{ padding: '12px 14px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ color: '#fff', fontSize: 12, fontWeight: 700, fontFamily: 'Poppins, sans-serif', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  Nossa música
                </p>
                <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {carta.nome_remetente} & {carta.nome_destinatario}
                </p>
              </div>
              <button onClick={() => setMinimizado(true)}
                style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', fontSize: 16, padding: 2, flexShrink: 0 }}>
                ↓
              </button>
            </div>
 
            <div style={{ height: 3, background: 'rgba(255,255,255,0.15)', borderRadius: 2, marginBottom: 8 }}>
              <div style={{ height: '100%', width: `${progresso}%`, background: '#fff', borderRadius: 2, transition: 'width 1s linear' }} />
            </div>
 
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 10 }}>{formatTime(segundos)}</span>
              <button onClick={togglePlay}
                ...
                {tocando
                  ? <svg width="12" height="12" viewBox="0 0 24 24" fill="#0d2d45"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>
                  : <svg width="12" height="12" viewBox="0 0 24 24" fill="#0d2d45"><path d="M8 5v14l11-7z"/></svg>
                }
              </button>
              <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 10 }}>-{formatTime(Math.max(duracao - segundos, 0))}</span>
            </div>
          </div>
        )}
      </div>
 
      {/* ✅ Iframe invisível — dimensões reais para autoplay funcionar */}
      {spotifyId && tocando && (
